...
std::optional<Results> getResultsWMatchFunc(QString leftImg,
    ...
    if (!leftImg.endsWith(".exrlayer")) { //if image is not exr, assume srgb, convert back to linear grayscale
        Multidim::Array<T_FV, 3> left_converted(imgLeft.shape()[0], imgLeft.shape()[1],1);
        ...
        for (int i = 0; i < imgLeft.shape()[0]; i++) {
            for (int j = 0; j < imgLeft.shape()[1]; j++) {
                ...
                for (int c = 0; c < imgLeft.shape()[2]; c++) {
                    val += std::pow(imgLeft.valueUnchecked(i,j,c), 1/2.2); //gamma correction to convert to linear
                }

                val /= imgLeft.shape()[2];

                left_converted.atUnchecked(i,j,0) = val;
            }
        }

        imgLeft = left_converted;
    } else if (imgLeft.shape()[2] > 1) { //image is rgb, convert to grayscale
        Multidim::Array<T_FV, 3> left_converted(imgLeft.shape()[0], imgLeft.shape()[1],1);
    ...
    if (!rightImg.endsWith(".exrlayer")) { //if image is not exr, assume srgb, convert back to linear grayscale
        Multidim::Array<T_FV, 3> right_converted(imgRight.shape()[0], imgRight.shape()[1],1);
        ...
        for (int i = 0; i < imgRight.shape()[0]; i++) {
            for (int j = 0; j < imgRight.shape()[1]; j++) {
                ...
                for (int c = 0; c < imgRight.shape()[2]; c++) {
                    val += std::pow(imgRight.valueUnchecked(i,j,c), 1/2.2); //gamma correction to convert to linear
                }

                val /= imgRight.shape()[2];

                right_converted.atUnchecked(i,j,0) = val;
            }
        }

        imgRight = right_converted;
    } else if (imgRight.shape()[2] > 1) {
        Multidim::Array<T_FV, 3> right_converted(imgRight.shape()[0], imgRight.shape()[1],1);
    ...
    double features_build_time = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start).count();
    ...
    double costvolume_build_time = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start).count();

    start = std::chrono::high_resolution_clock::now();
    auto truncatedCostVolume = StereoVision::Correlation::truncatedCostVolume(cost_volume, rawDisp, 0, 0, 1);
    stop = std::chrono::high_resolution_clock::now();

    double trunccostvolume_build_time = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start).count();
    ...
    stop = std::chrono::high_resolution_clock::now();
    double refine_parabola_time = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start).count();
    start = std::chrono::high_resolution_clock::now();
    ...
    stop = std::chrono::high_resolution_clock::now();
    double refine_equiangular_time = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start).count();
    ...
    stop = std::chrono::high_resolution_clock::now();
    double processedfeatures_build_time = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start).count();
    ...
    stop = std::chrono::high_resolution_clock::now();
    double refine_symmetric_time = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start).count();
    ...
    stop = std::chrono::high_resolution_clock::now();
    double refine_image_time = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start).count();
    ...
    stop = std::chrono::high_resolution_clock::now();
    double refine_predictive_time = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start).count();

	Multidim::Array<float,2> refinedImagePreNormalized;
	Multidim::Array<float,2> refinedPredictivePreNormalized;

    double refine_imageprenorm_time = std::nan("");
    double refine_predictiveprenorm_time = std::nan("");
    ...
    if (StereoVision::Correlation::MatchingFunctionTraits<matchFunc>::Normalized) {
        ...
        stop = std::chrono::high_resolution_clock::now();
        refine_imageprenorm_time = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start).count();
        ...
        stop = std::chrono::high_resolution_clock::now();
        refine_predictiveprenorm_time = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start).count();
    }
    ...
    Multidim::Array<T_FV, 3> const&  right_features_splines = (StereoVision::Correlation::MatchingFunctionTraits<matchFunc>::ZeroMean) ? right_features_zeromean : right_features;
    Multidim::Array<T_FV, 3> const&  left_features_splines = (StereoVision::Correlation::MatchingFunctionTraits<matchFunc>::ZeroMean) ? left_features_zeromean : left_features;
    ...
    auto refinedBicubicSplines =
            ...
            (left_features_splines,
             right_features_splines,
             rawDisp,
             nPixelsCut);
    stop = std::chrono::high_resolution_clock::now();
    double refine_splines_time = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start).count();
    ...
    ret.imagePreNorm.time = processedfeatures_build_time + refine_imageprenorm_time;
    ret.predictivePreNorm.time = processedfeatures_build_time + refine_predictiveprenorm_time;
...
int main(int argc, char** argv) {
    ...
    out << "time raw [ms]" << ',';
    out << "delta time parabola [ms]" << ',';
    out << "delta time equiangular [ms]" << ',';
    out << "delta time symmetric [ms]" << ',';
    out << "delta time image [ms]" << ',';
    out << "delta time image pre-norm [ms]" << ',';
    out << "delta time predictive [ms]" << ',';
    out << "delta time predictive pre-norm [ms]" << ',';
    out << "delta time bicubic [ms]" << Qt::endl;
    ...
    for (StereoVision::Correlation::matchingFunctions matchFunc : matchinFunctions) {
        for (int radius : radiuses) {
            ...
            for (QString line = inStream.readLine(); !line.isNull(); line = inStream.readLine()) {
                ...
                out << ret->raw.time << ',';
                out << ret->parabola.time << ',';
                out << ret->equiangular.time << ',';
                out << ret->symmetric.time << ',';
                out << ret->image.time << ',';
                out << ret->imagePreNorm.time << ',';
                out << ret->predictive.time << ',';
                out << ret->predictivePreNorm.time << ',';
                out << ret->splines.time << Qt::endl;
