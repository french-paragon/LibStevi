...
AffineTransform<float> estimateQuasiRigidMap(Eigen::VectorXf const& obs,
	...
	for (int i = 0; i < iterationLimit; i++) {
		...
		A.block<1,3>(n_obs+5, 0) = (x.block<3,1>(3,0) + Eigen::Vector3f(0,1,0)).cross(x.block<3,1>(6,0) + Eigen::Vector3f(0,0,1)).transpose();
		A.block<1,3>(n_obs+5, 3) = (x.block<3,1>(0,0) + Eigen::Vector3f(1,0,0)).cross(x.block<3,1>(6,0) + Eigen::Vector3f(0,0,1)).transpose();
		A.block<1,3>(n_obs+5, 6) = (x.block<3,1>(0,0) + Eigen::Vector3f(1,0,0)).cross(x.block<3,1>(3,0) + Eigen::Vector3f(0,1,0)).transpose();
